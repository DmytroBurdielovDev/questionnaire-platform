...
import { getQuestionnaires, deleteQuestionnaire } from '../api/questionnaireApi';
...
const QuestionnaireCatalog = () => {
  ...
  const fetchData = async (pageNumber = 1) => {
    try {
      setLoading(true);
      const { data, totalPages } = await getQuestionnaires(pageNumber, LIMIT);
      ...
      <div className="w-full max-w-6xl">
        ...
        {loading ? (
          ...
          <div className="text-center text-gray-400 py-10">
            ...
            <p className="text-sm">Click "Create Quiz" to add your first one.</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {questionnaires.map((q) => (
                <div
                  key={q.id}
                  className="bg-gray-800 shadow-lg rounded-lg p-5 border border-gray-700 hover:shadow-xl transition"
                >
                  <h3 className="text-lg font-semibold text-white">{q.name}</h3>
                  <p className="text-gray-300 text-sm mb-3">{q.description}</p>
                  <p className="text-gray-400 text-sm mb-4">
                    📌 {q.questions?.length ?? 0} questions · ✅ {q.completionsCount ?? 0} completions
                  </p>
